...
import { plain, type Commands, type Reply } from '../commands.js';
...
export class DiscordSurface {
  ...
  private payload(reply: Reply): { content: string; files?: string[] } {
    const content = plain(reply.rich).slice(0, 2000);
  ...
  async broadcast(text: string, image?: string | null): Promise<void> {
    ...
    if (ch && ch.isTextBased()) await (ch as TextChannel).send(this.payload({ rich: () => text, image }));
  }
  ...
  async start(): Promise<void> {
    this.client.on('messageCreate', async (m) => {
      ...
      if (reply.announce && this.o.channelId && m.channelId !== this.o.channelId) await this.broadcast(plain(reply.announce), reply.image);
    });
