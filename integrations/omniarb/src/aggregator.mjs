import { buildRouteGraph, searchRoutes } from './routes.mjs';
import { fresh } from './pricing.mjs';
export { assessNonAtomic, PaperNonAtomicLedger } from './nonatomic.mjs';
export { verifyDeployment, simulateRoundTrip } from './execution.mjs';
...
export async function researchRoutes(report, {
  ...
  for (const chain of report.chains) {
    ...
    pins[chain.chainId] = { chainId: chain.chainId, blockHash: chain.blockHash,
      stateRoot: chain.stateRoot, observedAt: chain.observedAt };
    for (const pool of chain.pools) {
      if (!pool.initialised) continue;
      const identity = `${chain.chainId}:${pool.poolManager.toLowerCase()}:${pool.id.toLowerCase()}`;
      pools.push({
        chainId: chain.chainId, manager: pool.poolManager, poolKey: pool.key,
        adapterId: Object.hasOwn(assignments, identity) ? assignments[identity] : 'unverified',
      });
    }
  }
  ...
  for (const search of searches) {
    try {
      results.push(await searchRoutes({
        ...search, graph, pins, simulateRoute, evaluate, now, maxAgeMs, mode: 'paper',
      }));
    } catch {
  ...
  return {
    ...
    supportedDirectedEdges: graph.edges.length, rejectedPools: graph.rejected,
    excludedChains, results,
  };
}
