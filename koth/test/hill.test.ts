...
describe('hill', () => {
  ...
  it('the pot is read from chain when someone wins: recorded on the challenge and in the awards ledger', async () => {
    ...
    const chain = new MemoryChain({ name: 'Master Shill', symbol: 'SHILL', uri: 'https://x/genesis.json' }, profiles);
    const h = new Hill({ judge: new MockJudge('power'), chain, uri: new MemoryUriProvider(), entry: new CountingEntry(), store: new MemoryStore(), potUsd: async () => onChain, uncontestedHandicap: 0 });
...
describe('king handicaps: an uncontested crown and erosion', () => {
  const D = 'D'.repeat(32);
  ...
  function siege(opts: { erosionPerLoss?: number; erosionMax?: number; uncontestedHandicap?: number } = {}) {
    ...
    return new Hill({ judge: new MockJudge('power'), chain, uri: new MemoryUriProvider(), entry: new CountingEntry(), store: new MemoryStore(), ...opts });
  }
  it('an uncontested first king fights at a handicap and loses ties', async () => {
    ...
    expect(h.handicap()).toEqual({ total: 10, erosion: 0, uncontested: 10, failedDefenses: 0 });
    ...
    expect(out.record.result).toBe('won');
    expect(out.record.verdict?.scores?.incumbent.handicap).toBe(10);
  ...
  it('every failed challenge erodes the king until someone breaks through, capped', async () => {
    const h = siege({ uncontestedHandicap: 0, erosionPerLoss: 20, erosionMax: 40 });
    await h.challenge({ mint: C, pitch: 'gamma', author: 'p', surface: 'cli' });
    for (let i = 0; i < 2; i++) expect((await h.challenge({ mint: B, pitch: 'beta tries', author: 'b', surface: 'cli' })).record.result).toBe('lost');
    expect(h.handicap()).toEqual({ total: 40, erosion: 40, uncontested: 0, failedDefenses: 2 });
    expect((await h.challenge({ mint: B, pitch: 'beta again', author: 'b', surface: 'cli' })).record.result).toBe('lost');
    expect(h.handicap().erosion).toBe(40);                                         // capped
    // Gamma 77 minus 40 = 37; Beta (26) still loses; Alpha (59) breaks through, and the new reign starts clean
